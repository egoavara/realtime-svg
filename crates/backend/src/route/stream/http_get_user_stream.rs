...
pub async fn handler(
    ...
    let session_for_log = format!("{}:{}", user_id, session_id);
    tokio::spawn(async move {
        ...
        while let Some(msg) = on_message.next().await {
            let payload: String = match msg.get_payload() {
                Ok(payload) => payload,
                Err(err) => {
                    error!(session = %session_for_log, %err, "Redis 메시지 페이로드를 읽지 못했습니다");
                    continue;
                }
            };
            ...
            match serde_json::from_str::<SvgFrame>(&payload) {
                ...
                Err(err) => {
                    error!(session = %session_for_log, %err, "Redis 메시지를 SVG 프레임으로 역직렬화하지 못했습니다");
                }
            }
        }
    });
